...
<title>電信框架管理平台</title>
	...
	<script type="text/javascript">
			$(function(){
				...
				//普通用户
				var normalNodes=[
					{ id:2, pId:1,page:'<%=request.getContextPath()%>/oper/businessAffairsVip/toViplist.do', name:"普通用户菜单1",icon:'<%=request.getContextPath()%>/ztree/css/zTreeStyle/img/diy/11.png'},
					{ id:4, pId:1,page:'<%=request.getContextPath()%>/oper/businessAffairsVip/topwdReset.do', name:"普通用户菜单2",icon:'<%=request.getContextPath()%>/ztree/css/zTreeStyle/img/diy/11.png'},
					{ id:6, pId:1,page:'<%=request.getContextPath()%>/oper/businessAffairsVip/toVipAuthentication.do', name:"普通用户菜单3",icon:'<%=request.getContextPath()%>/ztree/css/zTreeStyle/img/diy/11.png'}
	                   ];
	            //有部分权限用户
				var permissionNodes=[
                    <shiro:hasPermission name="permission">{ id:2, pId:1,page:'<%=request.getContextPath()%>/oper/user/topermissionInfo.do',name:"有权限用户菜单1",icon:'<%=request.getContextPath()%>/common/ztree/css/zTreeStyle/img/diy/11.png'},</shiro:hasPermission>
                    { id:3, pId:1,page:'<%=request.getContextPath()%>/oper/user/topermissionInfo1.do',name:"有权限用户菜单2",icon:'<%=request.getContextPath()%>/common/ztree/css/zTreeStyle/img/diy/11.png'}
					                  ];
				//有用户管理权限
				var userzNodes=[
					{ id:2, pId:1,page:'<%=request.getContextPath()%>/oper/user/tolistUserInfo.do', name:"用户列表",icon:'<%=request.getContextPath()%>/common/ztree/css/zTreeStyle/img/diy/11.png'},
					{ id:3, pId:1,page:'<%=request.getContextPath()%>/oper/privilege/toprivilegeInfo.do',name:"权限管理",icon:'<%=request.getContextPath()%>/common/ztree/css/zTreeStyle/img/diy/11.png'},
					{ id:6, pId:1,page:'<%=request.getContextPath()%>/oper/role/toListRoleInfo.do',name:"角色管理",icon:'<%=request.getContextPath()%>/common/ztree/css/zTreeStyle/img/diy/11.png'}
	                   ];
...
<div id="cc" class="easyui-layout" fit=true style="width:100%;height:100%;">  
		...
		<div>
			<img src="<%=request.getContextPath()%>/common/style/images/telecom.jpg" style="width:120px;height:80px;padding:0px;margin-left:10px;vertical-align:middle;"/>  
		...
		<div id="layout_north_kzmbMenu" style="width: 100px; display: none;">
		    ...
		    <div region="east" collapsed=true iconCls="icon-reload" title="East" split="true" style="width:100px;"></div>  
		     -->
		    <div region="west"  data-options="iconCls:'icon-tip',region:'west',split:true,collapsible:false,title:'菜单导航'"  style="width:200px;">
		    ...
		    <div data-options="region:'center'">
				<div id="tt" fit="true" class="easyui-tabs" border="false">
					<div title="系统公告" id="subWarp"
						...
						<iframe src="<%=request.getContextPath()%>/oper/user/toHome.do" style="width:100%;height:100%;border:0;"></iframe>
						<%--				这里显示公告栏、预警信息和代办事宜--%>
					</div>
				</div>
	     </div>
		  <div data-options="region:'south'"  style="height:50px;"><center style="padding-top: 10px">版权所有</center></div>
		</div>
